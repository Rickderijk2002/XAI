...
User testing followed the project protocol **CF Game User Testing Protocol (v3)** (`Report/Design Market/CF_Game_Test_Protocol.pdf`). The approach was a think-aloud usability test, followed by a short probing interview after the session. \newline
...
This participant has prior experience with counterfactuals and XAI in general, as well as some knowledge of this counterfactual assignment specifically. As such, the primary insights gained are relevant to the design of the dashboard and game. In particular, the use of a slider for the user's validity score was deemed misleading, since the objective validity score is given as a binary value (meaning the user validity will almost never match the objective validity). In addition, the participant reported having difficulty remembering which counterfactual was which, particularly between the initially shown CF, the user-chosen best CF, and the objective best CF, which are the three most important CFs for the game. Some steps were also evidently overwhelming, with the participant being unsure how much of the on-screen text was necessary to read; the participant ended up skipping large sections of text as the game went on. In step 4, three objective scores are shown (validity, plausibility, and implausibility). The first two are shown with a comparison to user score, but the third is not, since the user never gives their own estimate for implausibility. The participant deemed this confusing. Lastly, the participant expressed some frustration at the UI in terms of the "next" button. For this button to become active, the user has to first fill in the final text box, then explicitly either click outside of the text box or press control+enter. The participant felt this was not very clear and a minor but annoying extra step of effort.
...
Tim produced rich think-aloud material on metric directions and ranking. He flagged copy that sounded contradictory between plausibility and implausibility (“for both, higher is bad?”), asked for IM1 to state whether “realistic” means any digit or the target digit, and requested counterfactual images alongside metrics when rankings were discussed so validity differences would not rely on memory. He also noted a failed method slot in the five-way comparison. A useful quote anchor is: “Implausibility, higher is bad. But wait, it also says "high plausibility is bad", that doesn't add up.” The formatted source transcript ends after two closing prompts, so not every protocol interview sub-question appears as a separate heading in that file.

## Insights

1.  **Clarify validity early as a model prediction**, not only “looks like the target,” then align **slider** or feedback so users are not surprised when **binary** model validity appears (Patryk, Esmee, Tim, Andres).
2.  **Tighten metric copy and pairing**: separate **plausibility** and **implausibility**, state whether scores judge **target class** versus **any digit**, and keep **images beside rankings** or comparisons (Patryk, Tim, Esmee, Andres).
3.  **Reduce text and scroll distance** between guidance and tasks; add **hover or short inline** explanations for metrics (Max, Patryk, Tim).
4.  **Fix interaction affordances** for advancing after text entry and for **empty** method outputs so comparisons stay interpretable (Patryk, Andres).
5.  **Document assistance** in the report when drawing conclusions, because **all five** sessions included facilitator help at least once, which shapes what the UI can be said to teach on its own.
